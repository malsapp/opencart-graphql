<?php
class ControllerExtensionPhotoGallery extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$total = $this->model_extension_photo_gallery->getTotalPhotos();
		...
		$all_photos = $this->model_extension_photo_gallery->getAllPhotos($filter_data);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_extension_photo_gallery->editPhoto($this->request->get['photo_id'], $this->request->post);		
...
ini_set('display_errors', 1);
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_extension_photo_gallery->addPhoto($this->request->post);		
		...
		if (isset($this->request->get['photo_id'])) {
			$photo = $this->model_extension_photo_gallery->getPhoto($this->request->get['photo_id']);
		} else {
			$photo = array();
		}
		...
		if (isset($this->request->post['photo'])) {
			...
			$data['photo'] = $this->model_extension_photo_gallery->getPhotoDescription($this->request->get['photo_id']);
		} else {
			$data['photo'] = '';
		}
		...
		if (isset($this->request->post['image'])) {
			...
			$data['thumb'] = $this->model_tool_image->resize($photo['image'] ? $photo['image'] : 'no_image.png', 100, 100);
		} else {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $photo_id) {
				$this->model_extension_photo_gallery->deletePhoto($photo_id);
			}
	...
	protected function validateDelete() {
		if (!$this->user->hasPermission('modify', 'extension/photo_gallery')) {
